...
isUniformGRate=0;%use different gravity in gravitySediment
fiberDepth=1;
topSpaceHeight=1;%space height above the surface, used in shock2, [m]

%--------------step1: initializing Box model------------
B=obj_Box;%build a box object
if is2D==1
    sampleW=10;sampleL=0;sampleH=4;%width and length of the model, height will be determined by totalM
    ballR=2e-2;
else
    sampleW=6;sampleL=6;sampleH=4;%width and length of the model, height will be determined by totalM
    ballR=3.5e-2;
end
B.name='ShockDAS';
B.GPUstatus='auto';
...
B.setUIoutput();
B.SET.fiberDepth=fiberDepth;
B.SET.topSpaceHeight=topSpaceHeight;
