...
d.delElement(d.GROUP.sample(d.mo.aX(d.GROUP.sample)>0));
load('TempModel/fiber.mat');
fiberObj2.groupId=-10-fiberObj2.groupId;
fiberObj2=mfs.align2Value('bottom',mfs.moveObj2Origin(fiberObj2),0);
fiberObj2=mfs.align2Value('right',fiberObj2,0.8*B.sampleW/2);
d.GROUP.Fiber=d.addElement(1,fiberObj2);
% d.setGroupId;d.show('groupId');
...
for ii=1:totalCircle
    ...
    d.figureNumber = d.show('SETcData');
...
fs.movie2gif([B.name '_Fiber.gif'],frames,1/5);
