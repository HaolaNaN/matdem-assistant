...
indexBegin=1;%begin index of data file
indexEnd=30;%end index of data file
interval=1;%interval of data file
list=0:interval:indexEnd;
GIFType='aR';
GIFFigureNumber=1;
for showIndex=1:length(list)
    ...
    %xlim([0,B.sampleW]);zlim([0,B.sampleH]);
    %view(10*showCircle,30);%change the camera position
    set(gcf,'Position',[10,10,600,600]);%set the size of the figure
