...
%---------------------define parameters----------------------
ballR=0.1;%0.1 big ball, 0.03 small ball
frameValue=4;
%frame=[-frameValue,-frameValue,0,frameValue,frameValue,0];
frame.minX=0;
frame.minY=0;
frame.minZ=0;
frame.maxX=frameValue;
frame.maxY=0;
...
mo=d.mo;
d.GROUP.soil=[];
coor=(0<d.mo.aX&d.mo.aX<4) & (0<d.mo.aZ&d.mo.aZ<1.5);
d.addGroup('soil',find(coor),1);
d.GROUP.sample = setdiff(d.GROUP.sample, d.GROUP.soil);
d.mo.aR(d.GROUP.sample)=d.mo.aR(d.GROUP.sample)*0.5;
delFilter = sqrt((d.mo.aX - seedX).^2 + (d.mo.aZ - seedZ).^2) > 0.46 & ~coor;
...
clear;
[B,d]=fs.loadData('SeedDevice1.mat');%load data
seedX=1;seedZ=2;
d.delElement('topPlaten');
%ring
ringR=0.5;ringH=0.2;ballR=0.01;holeNum=4;holeAngle=30;
W1=holeAngle*pi*ringR/180; L=ringH;H=0.2;W2=holeAngle*pi*(ringR+2*ballR)/720;outL=0.2;
ring=makeRingWithHoles(ringR,ringH,ballR,holeNum,holeAngle);
ring=mfs.rotate(ring,'YZ',-90);
ring=mfs.move(ring,seedX,0,seedZ);
%scoop
scoop=makeHollowInvertedHopper(W1,L,H,W2,outL,ballR);
scoop=mfs.move(scoop,ringR+ringH,0,0);
scoopAll=mfs.rotateCopy(scoop,360/holeNum,holeNum,'XZ');
scoopAll=mfs.move(scoopAll,seedX,0,seedZ);
deviceObj=trifs.combineTriObj(scoopAll,ring);
device=triangle();
device.addGroup('device',device.addTriangle(deviceObj));
device.setMaterial(mean(d.mo.aKN(1:end-1)));
d.addNewTriangle(device);
% ringId=device.addTriangle(ring);
% device.addGroup('ring',ringId);
% device.setMaterial(mean(d.mo.aKN(1:end-1)));
% d.addNewTriangle(device);
% 
% scoopTri=triangle();
% scoopTri.addGroup('scoop',scoopTri.addTriangle(scoopAll));
% scoopTri.addTriCenter(device.aPointX(end),0,device.aPointZ(end));
% scoopTri.setMaterial(mean(d.mo.aKN(1:end-1)));
% %scoopTri.aPointX(end)=scoopTri.aPointX(end-1);scoopTri.aPointZ(end)=scoopTri.aPointZ(end-1);
% d.addNewTriangle(scoopTri);

% device.addGroup('all',[device.GROUP.scoopAll;device.GROUP.ring]);
% return

d.minusGroup('soil','sample',5);
d.show('aR');
d.showTri();
...
mAll.Ts=[0,4];%second
mAll.Xs=[0,1];%displacement
mAll.RXZs=[0,60];%degree

% nAll=Tool_Motion(d.tri{1},'ring');
% nAll.Ts=[0,4];%second
% nAll.Xs=[0,1];%displacement
% nAll.RXZs=[0,60];%degree
